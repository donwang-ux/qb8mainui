...
function SoSo_SmartBox(target, sCallBack, rChlCallBack){
    ...
    //window.sugCls.handleContent= soso_jpcall_inner;
...
*/
    ...
    (function(){
        ...
        */
        ...
        $("#smart_input").click(function(e) {

        	if($("#smart_input").hasClass('open') == false) {
        		$("#smart_input").addClass('open');
                isClick_ = true;
        	}
        	//don**如果没有open，且hover超过2秒，则open并取得键盘输入焦点
        	e.stopPropagation();//不会点到document
            if (isClick_) {
                $("#smart_input").select();//don,全选
                isClick_ = false;
            }
...
var initSearchEngine = function() {
    smartboxView = new SoSo_SmartBox(
        $('#smart_input')[0], function() {
            doSearch();
        },  function() {
            return true;
        });;
    container = $('div.searchbox');
    ...
    $(document).click(function(){
        ...
        //don收起搜索栏
  //      alert('点到document了');
        $("#smart_input").removeClass('open');
        $('#poparea').addClass("hide");
        //don end
    });
    //$('#smart_input').focus().val();
...
var changeLogo = function(channel, engin) {
  ...
  if (logoConfig[engin]) {
    ...
    //enginSelect.parent().hide();
    $("#searchdropdown").hide();
    //$('#smart_input').attr("placeholder",logoConfig[engin].title);
...
//don&photo,点击搜索栏，将展开。此逻辑同时用于处理展开收起相关的一些交互事件
//$( document ).on("click", "#smart_input", function() {
