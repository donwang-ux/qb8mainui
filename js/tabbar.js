	...
	function newtab(color,url,title,favicon){
		// $('<li class="tab tab_back" tabcolor="browserdefault"><img class="favicon" src="welcome/favicon.png"><span class="tabtitle">常用网站</span><div class="tabclose"></div></li>').insertBefore($("#tabList"));
		$('#tabs').append('<li class="tab tab_back" tabcolor="'+color+'"><img class="favicon" src="'+favicon+'"><span class="tabtitle">'+title+'</span><div class="tabclose"></div></li>');
		// $('#web').append('<iframe src="newtab/newtab.html" width="100%" height="100%" class="frame"></iframe>');
		$('#web').append('<iframe src="'+url+'" width="100%" height="100%" class="frame"></iframe>');
		showhide();
		marktab();
		// tabsnumber();
	}
	...
	function tabslayout(){
		...
		//平均分配tab宽度
		var tabwidth = ((tabswidth - 4 + 3)/tabnumber - 3);//首tab的左margin
		...
		if($('.tab').outerWidth() >= 262){
			// alert('232');
			$('.tab').outerWidth(262);
			$("#tabs").removeClass("max");
			$('#tabs').width(($('.tab').outerWidth() + 3) * tabnumber + 4);//首tab的左margin
			$('#tabList').addClass('hide');
			// $("#tabs").width($('.tab').outerWidth() * tabnumber);
			// $("#tabs").width($('.tab').outerWidth() * tabnumber);
		}else if($('.tab').outerWidth() < 262 && $('.tab').outerWidth() > 120){//如果标签宽度小于默认宽度，也就是标签受到挤压
			// alert('100-232');
			$("#tabs").addClass("max");
			if($("#browser").hasClass('tabup')){
				$("#tabs").outerWidth( $("#tabbar").width() - $("#newTab").outerWidth(true));
			}else{
				$("#tabs").outerWidth( $("#tabbar").width() - $("#newTab").outerWidth(true) - 50 );
			}
			$('.tab').outerWidth(($("#tabs").width()/$('.tab').size()) - 3);
			...
			if($("#browser").hasClass('tabup')){
				...
				$("#tabs").outerWidth( $("#tabbar").width() - $("#newTab").outerWidth(true) - $("#tabList").outerWidth(true) - 50);
			}
			$('.tab').outerWidth(($("#tabs").width()/$('.tab').size() - 3));
			// $('.tab').outerWidth(Math.floor($("#tabs").width()/$('.tab').size()));
		}
	...
	function framechange(){
		...
		if (frameurl.match("http://")){
			frameurl = frameurl.replace("http://","")
			$("#omniinput").val(frameurl);
		}else{
			$("#omniinput").val(frameurl);
		}
		//计算安全铭牌的left
		// var safex = (($('#omniinput').val().length + 2) * 8 );
		// console.log(safex);
		// $('.omnibtn_safe').css({left:safex});
		// alert("framechange");
	}
	//单击切换当前tab
	//function tabclick(){
  ...
  //   });
	...
	$('#newTab').mousedown(function(){
		// newtab();
		newtab('browserdefault','newtab/index.html','新页面','welcome/favicon.png');
		var tabnumber = $('.tab').size();
		//获取tab容器宽度
		var tabswidth = $('#tabs').width();
		//平均分配tab宽度
		var tabwidth = ((tabswidth - 4 - 3 * tabnumber)/tabnumber);//首tab的左margin
		// var tabwidth = Math.floor((tabswidth - 4)/tabnumber);//首tab的左margin
		$('.tab').outerWidth(tabwidth);
	...
	//恢复关闭tab
	function undotab(){
		if(closedTabNumber == 0){
			// alert('0');
			$('<li class="tab tab_current" tabcolor="'+closedTabColor+'"><img class="favicon" src="'+closedTabIcon+'"><span class="tabtitle">'+closedTabTitle+'</span><div class="tabclose"></div></li>').insertBefore($('#tabs').children().eq(closedTabNumber));
			$('<iframe src="'+closedTabUrl+'" width="100%" height="100%" class="frame showframe"></iframe>').insertBefore($('#web').children().eq(closedTabNumber));
		}else{
			// alert('not 0');
			$('<li class="tab tab_current" tabcolor="'+closedTabColor+'"><img class="favicon" src="'+closedTabIcon+'"><span class="tabtitle">'+closedTabTitle+'</span><div class="tabclose"></div></li>').insertAfter($('#tabs').children().eq(closedTabNumber - 1));
			$('<iframe src="'+closedTabUrl+'" width="100%" height="100%" class="frame showframe"></iframe>').insertAfter($('#web').children().eq(closedTabNumber - 1 ));
		}
		activateTab($($('#tabs').children().eq(closedTabNumber)));
		showhide();
		marktab();
		tabslayout();
	}
	$('#undoTab').click(function(){
		...
		// alert(closedTabNumber);
	});
	$('#btn_undotab').click(function(){
		if( $(this).hasClass('disabled') ){
	...
	function closeTab($tab, callback){
		//记录此tab的信息
		$('#undoTab').removeClass('disabled');
    	closedTabNumber = $tab.index();
    	closedTabIcon = $tab.children('.favicon').attr('src');
    	closedTabTitle = $tab.children('.tabtitle').html();
    	closedTabColor = $tab.attr('tabcolor');
    	closedTabUrl = $('#web').children().eq(closedTabNumber).attr('src');
...
/*	$('.tabclose').bind("click",function(){
    ...
    $('#tabs').on('click', '.tabclose', function() {
        ...
        if($(this).hasClass("tab_back")){
    		$(this).css({
			   'background-color' : lightcolorconfig[$(this).attr("tabcolor")],
